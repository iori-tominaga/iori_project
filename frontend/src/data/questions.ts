    ...
    questions: [
      {
        ...
        explanation: 'テスト計画書には、テストのスコープ（何をテストするか）、アプローチ（どのようにテストするか）、必要なリソース（人員・環境・ツール）、スケジュールを定義します。詳細なテストケースはテスト設計フェーズで作成されます。（ダミー解説）',
      },
      {
        ...
        explanation: 'テストモニタリングはテスト進捗を継続的に追跡してテスト計画との差異を把握する活動です。テストコントロールはその差異に基づいて是正行動を取る活動です。両者は継続的に行われます。（ダミー解説）',
      },
      {
        ...
        explanation: 'リグレッションテスト（回帰テスト）は、コードの変更（バグ修正・機能追加・リファクタリングなど）により、以前は正常に動作していた機能が影響を受けていないことを確認するテストです。（ダミー解説）',
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        explanation: 'リスクベーステストでは、リスクレベル＝発生可能性×影響度として評価します。発生可能性が高く影響度も大きいリスクから先にテストすることで、限られたリソースを効果的に配分できます。（ダミー解説）',
      },
      {
        ...
        explanation: 'ワイドバンドデルファイ法では、複数の専門家が各自独立して見積もりを行い、見積もり結果を共有した後にディスカッションを重ねて合意に達します。多様な視点を取り入れることで精度が向上します。（ダミー解説）',
      },
      {
        ...
        explanation: 'テストの独立性は「開発者本人 < 別の開発者 < 組織内の独立したテストチーム < 外部の独立したテスト機関」の順に高くなります。独立性が高いほど先入観なく欠陥を発見しやすい反面、コストも増加します。（ダミー解説）',
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        explanation: 'インスペクションのプロセスは「計画 → キックオフ（任意）→ 個人レビュー（準備）→ レビューミーティング → 修正 → フォローアップ」の順で実施されます。（ダミー解説）',
      },
      {
        ...
        explanation: 'モデレーターはレビューのファシリテーターとして、計画立案・参加者調整・ミーティング進行・フォローアップを担当します。欠陥修正は作成者（著者）が行います。（ダミー解説）',
      },
      {
        ...
        explanation: 'レビューの最大メリットは早期欠陥発見です。欠陥修正コストは後工程になるほど増大するため、要件・設計段階でのレビューによる欠陥除去は非常に費用対効果が高いです。（ダミー解説）',
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        explanation: '重大度（Severity）はシステムへの技術的影響の大きさ、優先度（Priority）はビジネス観点からの修正の緊急度を示します。両者は必ずしも一致しません。（ダミー解説）',
      },
      {
        ...
        explanation: '欠陥トリアージは、欠陥の優先度付け・対応方針・担当者アサインを関係者が協力して決定するミーティングです。限られたリソースを重要な欠陥修正に集中させます。（ダミー解説）',
      },
      {
        ...
        explanation: '欠陥の分類・傾向分析と根本原因分析（RCA）により、プロセスの問題点を特定して将来の欠陥混入を防ぐ改善策を講じることができます。（ダミー解説）',
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        explanation: 'TMMiは5段階：レベル1「初期」→レベル2「管理された」→レベル3「定義された」→レベル4「測定された」→レベル5「最適化」の順です。（ダミー解説）',
      },
      {
        ...
        explanation: 'TPI Next（Test Process Improvement Next）は16のキーエリアを定義し、各キーエリアを3段階で評価するフレームワークです。個々のキーエリアを独立して改善できる柔軟性があります。（ダミー解説）',
      },
      {
        ...
        explanation: 'GAP分析では現在のテストプロセスの状態（As-Is）と目標状態（To-Be）の差異（ギャップ）を特定します。このギャップが具体的な改善施策立案のインプットになります。（ダミー解説）',
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        explanation: 'ツール選定では、組織の課題解決への適合性、既存環境との統合可能性、ライセンス・導入・維持コストを含む総所有コスト（TCO）の評価が最重要です。（ダミー解説）',
      },
      {
        ...
        explanation: '自動化すべきテストと手動で行うべきテストを見極め、自動化の目的と期待値を明確にすることが成功の鍵です。すべてのテストを自動化しようとすると維持コストが膨大になります。（ダミー解説）',
      },
      {
        ...
        explanation: 'CI/CD環境ではコードコミットのたびに自動でビルドとテストが実行されます。問題を早期発見でき修正コストを削減できます。ただし探索的テストなどの手動テストは依然として必要です。（ダミー解説）',
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        explanation: 'テスト独立性の最大メリットは、テスターが確証バイアスを持たずに客観的な視点で欠陥を発見できる点です。ただし独立性が高すぎるとコミュニケーションコストが増加するトレードオフがあります。（ダミー解説）',
      },
      {
        ...
        explanation: 'マズローの欲求階層説は「生理的欲求→安全の欲求→社会的欲求（所属と愛情）→承認（尊重）の欲求→自己実現の欲求」の5段階です。安全の欲求が満たされると社会的欲求が現れます。（ダミー解説）',
      },
      {
        ...
        explanation: 'アジャイル開発ではテスターはスプリント内でチームと協力し継続的にテストを実施します。テスト活動はスプリントに組み込まれており、チーム全員でソフトウェアの品質に責任を持ちます。（ダミー解説）',
      },
    ],
  },
] as const;
